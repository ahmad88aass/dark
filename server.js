...
app.post("/api/request-charge", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "تم إرسال طلب الشحن للإدارة!" });
  } catch (e) { res.json({ success: false, message: "خطأ في الإرسال" }); }
});// ===== REDEEM VOUCHER =====
app.post("/api/redeem", async (req, res) => {
  try {
    const { userId, code } = req.body;
    const vRaw = await client.get(voucher:${code.toUpperCase()});
    if (!vRaw) return res.json({ success: false, message: "كود الشحن غير صحيح" });
...
app.post("/api/buy", async (req, res) => {
  try {
    ...
    const responseMsg = isTelegram
      ...
      : "✅ تم استلام طلبك! ستصلك تفاصيل الخدمة قريباً.";
...
app.get("/api/order-status", async (req, res) => {
  try {
    ...
    res.json({ success: true, status: order.status, deliveredCode: order.deliveredCode || null, serviceName: order.serviceName });
...
app.post("/api/bot-webhook", async (req, res) => {
  ...
  try {
    ...
    if (text.startsWith("/addbalance")) {
      ...
      // إنشاء كود شحن خاص
      const code = "AMR" + Math.random().toString(36).substring(2, 8).toUpperCase();
      await client.set(voucher:${code}, JSON.stringify({ amount, targetUserId: uid, used: false, createdAt: new Date().toISOString() }));
